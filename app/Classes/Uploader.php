...
{  
    ...
    public function validate(Request $request, $file, array $rules = []){
        ...
        if ($request->hasFile($file) && $request->file($file)->isValid()) {
            ...
            $this->fillProps();// заполняем данными
    ...
    // кладёт в нужное место
    public function upload($basePath = null)
    {
        $basePath = $basePath ?? ltrim(config('file.storagePath', storage_path()) . '/' . config('file.storageSection', ''), '/');
        ...
        if (!File::exists($newPath)) {
            if (!File::makeDirectory($newPath, config('file.storagePermissions', 0755), true)) {
...
// заполняем данными массив- размер, имя, 
